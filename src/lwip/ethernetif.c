...
#define INTERFACE_THREAD_STACK_SIZE (650)
...
phy_t phy_driver;
phy_ioctx_t phy_ioctx = phy_ioctx_default;
...
/* don't set NETIF_FLAG_ETHARP if this device is not an ethernet one */
    ...
    int32_t err = Phy_RegisterBusIO(&phy_driver, &phy_ioctx);
    if (err < PHY_STATUS_OK)
        printf("PHY Driver error: cannot register bus IO: %ld\n", err);

    /* Initialize the PHY Driver ETH PHY */
    err = Phy_Init(&phy_driver);
    if (err < PHY_STATUS_OK)
        printf("PHY Driver error: cannot init phy: %ld\n", err);

    if (hal_eth_init_status == HAL_OK)
    {
        PHYLinkState = Phy_GetLinkState(&phy_driver);
        if (PHYLinkState < PHY_STATUS_OK)
            printf("PHY Driver warning: cannot get link state: %ld\n", PHYLinkState);
...
int32_t ETH_PHY_I2C_IO_ReadReg(uint32_t DevAddr, uint32_t RegAddr, uint32_t *pRegVal)
{
    ...
    if(HAL_I2C_Mem_Read(&hi2c2, DevAddr << 1, RegAddr, I2C_MEMADD_SIZE_16BIT, (uint8_t*)pRegVal, 1, 100) != HAL_OK)
        return -1;
...
int32_t ETH_PHY_I2C_IO_ReadReg16(uint32_t DevAddr, uint32_t RegAddr, uint32_t *pRegVal)
{
    ...
    if(HAL_I2C_Mem_Read(&hi2c2, DevAddr << 1, RegAddr, I2C_MEMADD_SIZE_16BIT, (uint8_t*)pRegVal, 2, 100) != HAL_OK)
        return -1;
...
int32_t ETH_PHY_I2C_IO_WriteReg(uint32_t DevAddr, uint32_t RegAddr, uint32_t RegVal)
{
    ...
    if(HAL_I2C_Mem_Write(&hi2c2, DevAddr << 1, RegAddr,I2C_MEMADD_SIZE_16BIT, (uint8_t*)&RegVal, 1, 100) != HAL_OK)
        return -1;
...
int32_t ETH_PHY_I2C_IO_WriteReg16(uint32_t DevAddr, uint32_t RegAddr, uint32_t RegVal)
{
    ...
    if(HAL_I2C_Mem_Write(&hi2c2, DevAddr << 1, RegAddr,I2C_MEMADD_SIZE_16BIT, (uint8_t*)&RegVal, 2, 100) != HAL_OK)
        return -1;
...
void ethernet_link_thread(void *argument)
{
    ...
    for (;;)
    {
        ...
        if (PHYLinkState < PHY_STATUS_OK)
        {
            LWIP_DEBUGF(NETIF_DEBUG, ("ethernet_link_thread: error: %ld\n", PHYLinkState));
            continue;
        }
